...
"""
  Author:  fyh
  Purpose: this is basic gan to create digit of mnist
  Created: 2018年01月10日
"""
...
class Discrimator(nn.Module):
    def __init__(self):
        ...
        #self.p1 = nn.MaxPool2d(4) # input(8*24*24), output(8*6*6)
        self.bn = nn.BatchNorm2d(8)
        self.con2 = nn.Conv2d(8,4,5) #output(4,20,20)
        self.fc = nn.Linear(4*20*20,1024)
    ...
    def forward(self,x):
        ...
        s = self.con2(s)
        s = self.dropout(self.fc(F.leaky_relu(s.view(-1,4*20*20),0.2)))
        s = self.out(F.leaky_relu(self.bn2(s),0.2))
        return F.sigmoid(s)
...
if __name__ == '__main__':
    ...
    for ep in range(epoch):
        for i,(real_x, label) in enumerate(dataloader):
            ...
            dloss = loss(discrimator(fake_x.detach()), fake_y) + loss(discrimator(real_x), real_y)
            dloss.backward()
            #nn.utils.clip_grad_norm(discrimator.parameters(),0.8)
            ...
            gloss = loss(dlabel,real_y)
            gloss.backward()
            #nn.utils.clip_grad_norm(generator.parameters(),0.8)
